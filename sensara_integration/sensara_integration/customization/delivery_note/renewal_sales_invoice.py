...
def create_subscription_payload(doc,action):
    ...
    product_details = frappe.get_doc("Product Bundle", doc.item_code)
    ...
    if isinstance(doc.start_date, datetime.datetime):
        start_timestamp = doc.start_date.isoformat()
        end_timestamp = doc.end_date.isoformat()
    if isinstance(doc.start_date, str):
        try:
            start_timestamp = datetime.datetime.strptime(doc.start_date, "%Y-%m-%d %H:%M:%S").isoformat()
            end_timestamp = datetime.datetime.strptime(doc.end_date, "%Y-%m-%d %H:%M:%S").isoformat()
        except Exception as e:
            start_timestamp = datetime.datetime.strptime(doc.start_date, "%Y-%m-%d").isoformat()
            end_timestamp = datetime.datetime.strptime(doc.end_date, "%Y-%m-%d").isoformat()		
    body = {
            "action": action,
